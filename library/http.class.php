...
class Http{
	...
	public static function post($key=null, $filter='trim', $default=null, $param='')
	{
		...
		return self::filter($post[$key], $filter, $default, $param);
	}
	...
	{		
		...
		return self::filter($request[$key], $filter, $default, $param);
	}
	...
	public static function get($key=null, $filter='trim', $default=null, $param='')
	{
		...
		return self::filter($get[$key], $filter, $default, $param);
	}
	
	private static function filter($value, $type='trim', $default=null, $param='')
	{
		if(is_array($value))
		{
			foreach($value as &$item)
			{
				$item = self::filter($item, $type, $default, $param);
			}
			return $value;
		}
		...
		switch($type)
		{
			...
			case 'float':
				...
				break;
			case 'date':
				$value = self::isDate($value, $default) ? $value : '';
				break;
			case 'String':
				break;
			default:
