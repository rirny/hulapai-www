...
class Push
{
	...
	*/
	...
	{		
		if($this->act == "delete"){
			...
			if($deleteExt){
				...
				if(!$_Delete->where($deleteLog, true)){
	...
	{		
		...
		{
			$student = load_model('student')->where('id', $this->student, true)->field('name,nickname')->Row();	
			$to = $student['name'];
		}else{ // 发给老师			
			$teacher = load_model('user')->where('id', $this->to, true)->field('firstname,lastname,nickname')->Row();
			$to = $teacher['firstname'] . $teacher['lastname'];
		}
		if($this->act == 'add')
		{			
			if(empty($this->ext['event'])) return false;
			$event = load_model('event')->where('id', $this->ext['event'], true)->field('id,text,start_date,end_date,rec_type,is_loop,teacher,school');			
			if($this->character == 'student')
			{
				$relation = load_model('student_course')
					->where('event', $this->ext['event'], true)
					->where('student', $this->student)
					->field('remark,start_date,end_date')->Row();
				...
				$relation = load_model('teacher_course')->where('event', $this->ext['event'], true)
					->where('teacher', $this->to)->field('remark')->Row();
			}
		...
		{
			if(empty($this->ext['event'])) return false;            
			$event = load_model('event')->where(id, $this->ext['event'], true)
				->field('id,text,start_date,end_date,rec_type,is_loop,teacher,school')
				->Row();			
			if($this->character == 'student')
			{
				$relation = load_model('student_course')
		...
		{
			$school = load_model('school')->where('id', $event['school'], true)->field('name')->Row();
			$character = '（机构）';
			$from = $school ? $school['name'] : '';
		}else
		{
			$teacher = load_model('user')->where('id', $event['teacher'], true)
				->field('firstname,lastname,nickname,hulaid')->Row();
